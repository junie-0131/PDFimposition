...
function insertDuplexSpread(pageCount, insertPages, settings) {
  // 中央ソースも末尾ソースも、差込2ページを同一面ドンテン配置で別版面に出力する
...
function updatePreview() {
  ...
  boxes.forEach((box, i) => {
    ...
    if (settings.printFolios && selected.pages[i]) {
      ...
      folio.textContent = `P${selected.pages[i] - 1 + (Number(settings.folioStart) || 1)}`;
...
function preflight(settings, count, plan) {
  ...
  if (settings.grain === "short" && settings.product !== "flat" && settings.binding !== "top") out.push({ level: "warn", text: "冊子物で横目指定です。背割れ・開き具合・折り方向を確認してください。" });
  if (settings.grain === "long" && settings.product !== "flat" && settings.binding === "top") out.push({ level: "warn", text: "天綴じで縦目指定です。背に平行な紙目（横目）を確認してください。" });
...
function drawFolio(page, pageNo, x, y, w, h, settings, font) {
  const folioStart = Number(settings.folioStart) || 1;
  const text = `P${pageNo - 1 + folioStart}`;
...
function creepOffset(settings, pageNo, pageCount) {
  ...
  // 外側の折丁(補正0)から最内側の折丁(最大補正)へ段階的にノド側へずらす
  const nearEdge = Math.min(pageNo, pageCount + 1 - pageNo);
  const sheetNumber = Math.ceil(nearEdge / 2);
  const totalSheets = Math.max(Math.ceil(pageCount / 4), 1);
  const fraction = totalSheets > 1 ? (sheetNumber - 1) / (totalSheets - 1) : 0;
  const direction = settings.binding === "right" ? 1 : -1;
  return direction * fraction * settings.creep;
}
